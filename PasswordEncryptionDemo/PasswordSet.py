...
with open("PasswordKey.key", "wb") as keyfile:
    keyfile.write(key)
...
def encrypt(check):
    while check:
        ...
        paasword_c = pyautogui.confirm(text='Is this your Password? \n\n' + password, title='???', buttons=['YES', 'NO'])

        if paasword_c == 'YES':
            password = bytes(password, 'utf-8') #For encryption the string must be tranformed to bytes
